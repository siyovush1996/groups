...
This repository holds the list of GitHub repositories used on `w3.org/groups/*/*/tools/` pages, such as the [CSS WG too/css/tools/) page.
...
* `owners` : an Array of the GitHub owner objects (to complete the [ones from the W3C DB](https://github.com/w3c/groups-server/blob/main/loop.js#L9)), as follows:
...
* `trustedOwners` : an Array of strings, identifying Github owners to be trusted to associate repositories to WGs.

Note: `owners` and `trustedOwners` are expanded at runtime by information coming from the W3C database.

The server code is currently running within labs.w3.org (titania).
